...
public ResponseEntity<?> saveUser(User user) throws MessagingException, IOException {
    ...
    String logoUrl = "https://infeedu.com/static/media/infeedu.72f9668800a49ae736c8.png";
    String confirmationUrl = "https://infeedu.com:8443/api/user/confirm-account?token=" + confirmationToken.getConfirmationToken();
    ...
    String htmlMessage = "<div style=\"font-family: Arial, sans-serif; color: #333; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #e0e0e0; border-radius: 8px; background-color: #f9f9f9;\">" +
            ...
            "Thank you for registering with <strong>Infeedu</strong>! To confirm your account and complete the registration process, please click the button below." +
            "</p>" +
            ...
            "<p>&copy; 2025 Infeedu. All rights reserved.</p>" +
    ...
    if (token != null) {
        ...
        if (user != null) {
            ...
            return ResponseEntity.status(302)
                    .header("Location", "https://infeedu.com/login")
                    .build();
        }
    }
    return ResponseEntity.badRequest().body("Error: Couldn't verify email");
